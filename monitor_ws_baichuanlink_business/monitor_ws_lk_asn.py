...
class MonitorWSLKASN(object):
	...
	def wslk_asn(cls, wsielog):
		...
		for h in machine_pv.keys():
			...
			ws_lk_server_asn.labels(region, province, ip, 'unkown').set(machine_asn)
		...
		for r in region_pv:
			...
			ws_lk_server_asn.labels(region, province, ip, 'unkown').set(region_asn)
